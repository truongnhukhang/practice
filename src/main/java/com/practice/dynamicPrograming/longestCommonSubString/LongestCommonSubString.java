...
public class LongestCommonSubString {
  public static void main(String[] args) {
    String s1 = "abdddd";
    String s2 = "abcddc";
    System.out.println(new LongestCommonSubString().findLongest(s1,s2,0,0,0,new Integer[s1.length()][s2.length()]));
  }
  ...
  public int findLongest(String s1,String s2,int idx1,int idx2,int count,Integer[][] dp) {
    ...
    int max2 = findLongest(s1,s2,idx1+1,idx2,count,dp);
    int max3 = findLongest(s1,s2,idx1,idx2+1,count,dp);
